...
- Обновен е алгоритъма за проверка на РЗИ номерa на практиката съгласно новата номерация от 02.2024 (благодарности на <b>д-р Виктория Будаковa</b>)
- Оправен е бъга при който НРН номерът на направлението не се запазва в базата данни
- Оправен е бъга при който последната цифра от НРН номера на направлението не се принтира в бланка МДД4
...
- Оправен е проблема при който програмата няма правомощия да зареди драйвърите за КЕП под macOS

## [1.11.2 (16.03.2024)](https://github.com/thefinalcutbg/DinoDent/releases/tag/v1.11.2)
- Оправен е краша при въвеждане на нов пациент (докладван от <b>д-р Веселин Георгиев</b>)
...
- Оправен е бъга при който програмата крашва при изпращане на манипулация Обща анестезия към НЗИС (докладван от <b>д-р Сребрина Соклева</b>)
...
- Оправен е бъга от версия 1.10.0, при който не се визуализира формуляра за издаване на медицински бележки
...
Оправени бъгове:
- Оправен е бъга при който ПИС не приема финансови документи от издател регистриран като едноличен търговец - ЕТ (докладван от <b>Петър Първанов</b>)
...
- Оправен е бъга при който не може да бъде отбелязана подвижност на свръхброен зъб
...
Оправени бъгове:
- Оправен е бъга при който програмата не открива някои електронни подписи (благодарности на всички докладвали проблема)
- Оправен е бъга при който при промяна на данните на лекаря, кодът по който той е сключил договор с НЗОК временно се премахва
...
Оправени бъгове:
- Оправен е бъга при който манипулациите от ПИС се прилагат към статуса в обратна хронология (докладван от <b>д-р Георги Славчев</b>)

## [1.8.4 (29.11.2023)](https://github.com/thefinalcutbg/DinoDent/releases/tag/v1.8.4)
Добавени функционалности:
- Актуализиран е списъка на населените места с неблагоприятни условия
...
- Оправен е бъга от версия 1.8.0, при който статус C(кариес) не фигурира в принтирания амбулаторен лист и месечния XML отчет към НЗОК
...
Оправени бъгове:
- Оправен е бъга, при който когато името на потребителя в Windows е на кирилица програмата не може да създаде правилно началната конфигурация и базата данни (благодарности на <b>д-р Мария Вълкова-Генчева</b> и на <b>д-р Александър Комсалов</b>)
...
- Оправен е бъга при който не се отваря стар амбулаторен лист, когато е отворен лист на същия пациент от текущия месец
- Оправен е проблема, при който при дисплей с висок DPI мащаб, прозорецът за онлайн пациентското досие излиза извън границите на екрана (докладван от <b>д-р Слави Бостанджиев</b>)
...
Оправени бъгове:
- Оправен е бъга при който при смяна на датата на манипулация изпратена към НЗИС, при корекция се подава и анулирано нейно копие с предишната дата
- Оправен е бъга при който при редактиране на манипулация на свръхброен зъб, отметката за свръхбройния зъб не е маркирана
- Оправен е бъга при който при издаване на направление към специалист (бл.119) с код 68 в XML отчета кодът фигурира като 64
- Оправен е бъга при който не се принтира формуляра на направление МДД4 при ОПГ, а само данните му
...
- Оправен е бъга при който при принтиране на направление за МДД последната цифра от ЕГН-то на пациента липсва (докладван от <b>д-р Сияна Граданска</b>)
- Оправен е бъга при който цената на допълнителния преглед за бременни по НЗОК не се включва в предварително изчислената сума за месечния отчет (докладван на <b>д-р Зорница Банкова</b>)
...
Оправени бъгове:
- Оправен е бъга при който понякога програмата забива при означаване на шинирани зъби като липсващи
- Оправен е бъга при презаписване на конфигурационния файл
- Оправен е алгоритъма за проверка на булстат с 13 цифри
...
Оправени бъгове:
- Оправен е текста на допълнителната информация, която се появява при посочване на зъбите с курсора на мишката
