...
DinoDent е безплатен дентален софтуер с отворен код. Замислен е да бъде от полза на всички лекари по дентална медицина. Всеки е свободен да го ползва за своите нужди, декомпилира или построява използвайки кода от [GitHub](https://github.com/thefinalcutbg/DinoDent){:target="\_blank"}.

## За кого е предвиден DinoDent?

DinoDent е насочен основно към индивидуални дентални практики. Може sда се използва и в клиники, но софтуерът не е разработен с тази цел. В него няма да откриете CRM или финансови функционалности като изчисляване на заплати, справки за приходи/разходи, складова наличност и възможност за връзка с касов апарат.
...
Под Windows DinoDent поддържа всички модели пен таблети одобрени от НЗИС. Към момента това са:
...
Kym момента таблетите не функционират на macOS поради липса на драйвери предоставени от производителите.

## Защо при амбулаторен лист подписан от пациента с пен таблет, не се визуализира изображението на подписа при разпечатването му или при запазването му като PDF?
Изображението на подписа само по себе си няма юридическа стойност като електронен подпис. При подписване с пен таблет подписът на пациента представлява криптографска информация принадлежаща към XML документа който се изпраща към НЗИС, а не към PDF файл или хартиена разпечатка. Затова поставянето на изображение на подписа върху амбулаторния лист би било технически подвеждащо и юридически некоректно.

Положения подпис може да бъде прегледан в DinoDent чрез клик върху изображението на подписа до бутона за изпращане към НЗИС и избор на раздел „Подпис“.
...
## Може ли да се изкарват различни справки от DinoDent?

Да. В Настройки → SQL DinoDent позволява изпълнение на SQL заявки към базата данни. По подразбиране се показва нейната структура, която може да запазите като CSV и да предоставите на ИИ инструмент, например ChatGPT. След това просто описвате каква справка искате и ИИ може да генерира необходимата SQL заявка, която да копирате обратно в DinoDent.
